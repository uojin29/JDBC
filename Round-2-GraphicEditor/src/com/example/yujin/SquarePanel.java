	...
	public void paintComponent(Graphics g) {
		if(sv.size() != 0){
			for(int i=0;i<se.size();i++){ //벡터크기만큼
				...
				Point ep = se.get(i);	
				g.drawRect(Math.min(sp.x, ep.x), Math.min(sp.y, ep.y),Math.abs(ep.x- sp.x),Math.abs(ep.y- sp.y));//그리다
			}
		}
		if(start == null) // 타원이 만들어지지 않았음
			return;
		g.drawRect(Math.min(start.x, end.x), Math.min(start.y, end.y), Math.abs(start.x - end.x),  Math.abs(start.y - end.y)); // 타원 그리기
	}
}
